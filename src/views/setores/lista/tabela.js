  ...
  const handleFilter = e => {
    ...
    if (value.length) {
      updatedData = data.filter(item => {
        const startsWith =
          item.setor.toLowerCase().startsWith(value.toLowerCase())

        const includes =
          item.setor.toLowerCase().includes(value.toLowerCase())
    ...
    <Card className="overflow-hidden">
      <CardHeader>
        <div className="d-flex justify-content-between w-100">
          ...
          <Button type="submit" color="primary" onClick={() => navigate("/setores/form")}>
            Novo Setor
          </Button>
        </div>
      </CardHeader>
