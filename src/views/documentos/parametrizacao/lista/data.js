...
const handleConfirmCancel = (id) => {
  return MySwal.fire({
    ...
    if (result.value) {
      httpRequest(`documentos/tipos/parse/${id}`, {
        method: "DELETE",
        token: getToken(),
      }).then(() => {
        MySwal.fire({
    ...
    cell: (row) => {
      return (
        <div className="d-flex">
          ...
          <Link className="text-primary ms-1" title="Editar"  to={`/documento/tipos/parametrizacao/form/${row.documento_tipo_id}/${row.id}`}>
            <Edit size={15} />
          </Link>
        </div>
      );
