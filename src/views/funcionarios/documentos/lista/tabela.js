...
const DataTablesReOrder = () => {
  ...
  useEffect(() => {
    httpRequest(`documentos?funcionario_id=${id}`, {
      method: "GET",
      token: getToken(),
    }).then(res => {
      setDados(res.body);
    ...
    <Card className="overflow-hidden">
      <CardHeader>
        <div className="d-flex justify-content-between w-100">
          <CardTitle tag="h4">Funcionário - {funcionario.nome}</CardTitle>
          <Button type="submit" color="primary" onClick={() => navigate(`/documentos/form?funcionario_id=${id}&empresa_id=${funcionario.empresa_id}`)}>
            Novo Documento
          </Button>
        </div>
      </CardHeader>
